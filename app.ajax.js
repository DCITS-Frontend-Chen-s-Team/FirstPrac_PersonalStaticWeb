...
(function (mui, owner) {
    ...
    mui.ajaxSettings.beforeSend = function (xhr, setting) {
        ...
        //console.log('app.ajax[beforeSend] setting:' + JSON.stringify(setting));
    };
    // 设置全局complete
    mui.ajaxSettings.complete = function (xhr, status) {
        //console.log('app.ajax[complete]status:' + status);
    }

    /**
     * 使用HTTP POST方式获取服务器JSON编码数据
     * @param {Object} options
     */
    var encStr = "dcitszlb";
    var macStr = "knablzst";
    owner.getJSONData = function (options) {
        var getJSONData;
        var switchFlag = true; // 是否加密 开关
        options = options || {};
        var data = createRequestData(options);
        var successFunc = options.success || mui.noop;
        var errorFunc = options.error || mui.noop;
        var url = getUrl();
        //可根据服务内容自设相应超时时间，默认为70s
        var overtime = 70000;
        if (options.timeout != undefined && options.timeout != '') {
            overtime = options.timeout;
        }
        var enc = encryptByDES(data, encStr);
        var mac = encryptByDES(data, macStr);
        var dataJson = JSON.parse(data);
        var dataJsonB = {};
        dataJsonB['SYS_HEAD'] = dataJson.SYS_HEAD;
        dataJsonB['LOCAL_HEAD'] = dataJson.LOCAL_HEAD;
        dataJsonB["enc"] = enc;
        dataJsonB["mac"] = mac;
        var SYS_HEAD = dataJsonB.SYS_HEAD;
        if (SYS_HEAD.SERVICE_CODE == "SMART_MOBILE" && SYS_HEAD.MESSAGE_TYPE == "2002" && SYS_HEAD.MESSAGE_CODE == "0001") {
            switchFlag = false;
        }

        if (switchFlag) {
            getJSONData = JSON.stringify(dataJsonB);
        } else {
            getJSONData = data;
        }
        ...
        mui.ajax(url, {
            ...
            success: function (res) {
                var result = res;
                if (switchFlag) {
                    if (res.RET_CODE == "666666") {
                        if (res.enc) {
                            result = JSON.parse(decryptByDES(res.enc, encStr));
                        }
                        successFunc(result);
                    } else {
                        var XMLHttpRequest = {};
                        XMLHttpRequest.status = res.RET_CODE;
                        XMLHttpRequest.msg = res.RET_INFO;
            ...
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                console.log("app.ajax[error]textStatus:" + textStatus + ",errorThrown:" + errorThrown);
    ...
    var getUrl = function () {
        //service服务地址
        var server = app.service.getSysUrl("SERVICE") || '';
        //下载地址
        var serverAddress = app.service.getSysUrl("DOWNLOAD") || '';
        //格式 http://188.177.155.163:8080
        app.storage.setURL(serverAddress);
        return server;
    }
    ...
    var createRequestData = function (options) {
        var data = getSysHead(options.service);
        var rdata = options.data || {};
        for (var item in rdata) {
            data[item] = rdata[item];
        }
        return JSON.stringify(data);
    }

    var getSysHead = function (code) {
        return app.service.getSysHead(code);
    }
    owner.connectWarnning = function (XMLHttpRequest, textStatus, errorThrown) {
        var msg = "网络无法连接，请稍后重试！";
        if (textStatus != null) {
        ...
        if (status == "404") {
            ...
            msg = XMLHttpRequest.msg || "网络无法连接，请稍后重试！";
        }
